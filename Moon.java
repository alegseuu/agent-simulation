...
class Moon extends MassObject {
    ...
    public Moon(int mass, int age, String name, double[] velocity, double[] position, int minDistance) {
    ...
    public void calculateNetForce(double[][] forces, int numPlanets) {
    ...
    public void calculateVelocity(MoonAttraction moon_attract) {
        this.velocity[0] =Math.sqrt(this.acceleration[0] * moon_attract.dx / 100);
        this.velocity[1] = Math.sqrt(this.acceleration[1] * moon_attract.dy /100);
    }

    public void nextPosition(double[] direction, MoonAttraction moon_at,
                             Planet[] planets, int numPlanets){
        double dis;
        boolean dist_check = false;
        for(int i = 0; i < numPlanets; i++){
            dis = moon_at.checkDistance(this, planets[i]);
